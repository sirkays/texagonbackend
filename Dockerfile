...
HEALTHCHECK --interval=30s --timeout=5s --retries=3 \
  CMD python -c "import socket; s=socket.socket(); s.settimeout(2); s.connect(('127.0.0.1', int('${PORT}')))"

ENTRYPOINT ["/app/entrypoint.sh"]

# Default (web) command — entrypoint will override based on PROCESS_TYPE
